...
#include "TrueFPSGameInstance.generated.h"

class FTrueFPSMainMenu;
class FTrueFPSWelcomeMenu;
class FTrueFPSMessageMenu;
class ATrueFPSGameSession;
...
public:
	...
	TWeakObjectPtr< ULocalPlayer > PlayerOwner;		// Owner of dialog who will have focus (can be nullptr)
};

class FTrueFPSPendingInvite
{
public:
	FTrueFPSPendingInvite() : ControllerId(-1), UserId(nullptr), bPrivilegesCheckedAndAllowed(false) {}
...
class STrueFPSWaitDialog : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(STrueFPSWaitDialog)
	{}
...
UCLASS(config=Game)
class TRUEFPSSYSTEM_API UTrueFPSGameInstance : public UGameInstance
{
	GENERATED_BODY()

public:

	UTrueFPSGameInstance(const FObjectInitializer& ObjectInitializer);

	bool Tick(float DeltaSeconds);

	ATrueFPSGameSession* GetGameSession() const;
	...
	void SetPendingInvite(const FTrueFPSPendingInvite& InPendingInvite);
...
private:
	...
	FTrueFPSPendingMessage PendingMessage;
	...
	void HandleDemoPlaybackFailure(EReplayResult, const FString& ErrorString );
