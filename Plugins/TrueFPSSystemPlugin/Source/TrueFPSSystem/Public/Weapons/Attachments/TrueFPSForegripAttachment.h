...
#include "TrueFPSForegripAttachment.generated.h"

/**
 * 
 */
UCLASS(Abstract, Meta = (DisplayName = "Foregrip Attachment")) 
class TRUEFPSSYSTEM_API ATrueFPSForegripAttachment : public ATrueFPSWeaponAttachmentBase
{
	GENERATED_BODY()
	
public:

	ATrueFPSForegripAttachment();

protected:
	
	virtual void OnRemoved() override;

	virtual FORCEINLINE void OnEquipped(class ACharacter* Character) override
	{ // On equipped, set hand placement transform after a tick so that the AnimPose is being played during the update.
		Super::OnEquipped(Character);
		SetHandPlacement();
	}
	...
	class UTrueFPSPoseableHandComponent* HandAttachPoint;
