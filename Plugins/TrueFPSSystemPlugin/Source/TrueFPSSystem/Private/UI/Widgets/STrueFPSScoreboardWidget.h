...
DECLARE_DELEGATE_RetVal_OneParam(int32, FOnGetPlayerStateAttribute, class ATrueFPSPlayerState*);
...
//class declare
class STrueFPSScoreboardWidget : public SBorder
{

	SLATE_BEGIN_ARGS(STrueFPSScoreboardWidget)
	{}

	SLATE_ARGUMENT(TWeakObjectPtr<APlayerController>, PCOwner)

	SLATE_ATTRIBUTE(ETrueFPSMatchState::Type, MatchState)
...
protected:
	...
	ATrueFPSPlayerState* GetSortedPlayerState(const FTeamPlayer& TeamPlayer) const;
	...
	int32 GetAttributeValue_Kills(class ATrueFPSPlayerState* PlayerState) const;

	/** get attribute value for deaths */
	int32 GetAttributeValue_Deaths(class ATrueFPSPlayerState* PlayerState) const;

	/** get attribute value for score */
	int32 GetAttributeValue_Score(class ATrueFPSPlayerState* PlayerState) const;
	...
	const struct FTrueFPSScoreboardStyle *ScoreboardStyle;
};
