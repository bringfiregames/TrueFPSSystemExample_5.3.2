...
#include "STrueFPSSplitScreenLobbyWidget.h"
...
#define LOCTEXT_NAMESPACE "TrueFPSSystem.SplitScreenLobby"

int32 GTrueFPSSplitScreenMax = 2;
static FAutoConsoleVariableRef CVarTrueFPSSplitScreenMax(
	TEXT("r.TrueFPSSplitScreenMax"),
	GTrueFPSSplitScreenMax,
...
void STrueFPSSplitScreenLobby::Construct( const FArguments& InArgs )
{
...
#if PLATFORM_SWITCH
	PressToPlayText = LOCTEXT("PressToPlay", "<img src=\"TrueFPSSystem.Switch.Right\"/> Select User");
	PressToFindText = LOCTEXT("PressToFind", "Press <img src=\"TrueFPSSystem.Switch.Right\"/> to Find Match");
	PressToStartMatchText = LOCTEXT("PressToStart", "<img src=\"TrueFPSSystem.Switch.Right\"/> Start Match / <img src=\"TrueFPSSystem.Switch.Up\"/> Connect Controllers");
	PlayAsGuestText = LOCTEXT("PlayAsGuest", "<img src=\"TrueFPSSystem.Switch.Left\"/> Play As Guest");
#endif	
	...
	const FTrueFPSMenuStyle* ItemStyle = &FTrueFPSStyle::Get().GetWidgetStyle<FTrueFPSMenuStyle>("DefaultTrueFPSMenuStyle");
...
//								.ColorAndOpacity(MenuTitleTextColor)
							...
							[							
								SNew(SRichTextBlock)
								.TextStyle(FTrueFPSStyle::Get(), "TrueFPSSystem.SplitScreenLobby.StartMatchTextStyle")							
								.Text(this, &STrueFPSSplitScreenLobby::GetPlayFindText)
								.DecoratorStyleSet(&FTrueFPSStyle::Get())
								+ SRichTextBlock::ImageDecorator()
							]
						]					
					...
					[
						...
						[
							...
							[							
								SNew(SRichTextBlock)
								.TextStyle(FTrueFPSStyle::Get(), "TrueFPSSystem.SplitScreenLobby.StartMatchTextStyle")							
								.Text(this, &STrueFPSSplitScreenLobby::GetPlayAsGuestText)
								.DecoratorStyleSet(&FTrueFPSStyle::Get())
								+ SRichTextBlock::ImageDecorator()
							]
						]	
...
void STrueFPSSplitScreenLobby::Clear()
{	
...
void STrueFPSSplitScreenLobby::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	// Parent tick
...
void STrueFPSSplitScreenLobby::UpdateSlots()
{
	if (GetGameInstance() == nullptr)
	{
		return;
	...
	{
		ULocalPlayer * LocalPlayer = ( i < GetGameInstance()->GetNumLocalPlayers() ) ? GetGameInstance()->GetLocalPlayerByIndex( i ) : nullptr;
...
void STrueFPSSplitScreenLobby::ConditionallyReadyPlayer( const int ControllerId, const bool bCanShowUI )
{
	UTrueFPSGameInstance* const GameInstance = GetGameInstance();
	...
	if (GameInstance->FindLocalPlayerFromControllerId( ControllerId ) != nullptr )
	{
		return;
	}
		...
		( bFoundExistingPlayer || !bIsPlayerOnline ) )
	{
		...
		{
			ExternalUI->ShowLoginUI( ControllerId, OnlineMode == EOnlineMode::Online, false, FOnLoginUIClosedDelegate::CreateSP( this, &STrueFPSSplitScreenLobby::HandleLoginUIClosedAndReady ) );
		}
			...
			if(Identity.IsValid())
			{
				...
				Identity->GetUserPrivilege(
					*UniqueNetId,
					...
					IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate::CreateSP(this, &STrueFPSSplitScreenLobby::OnUserCanPlay));
			}
		}
	}
	else
...
void STrueFPSSplitScreenLobby::ReadyPlayer( const int ControllerId )
{
	FString Error;
...
void STrueFPSSplitScreenLobby::UnreadyPlayer( const int ControllerId )
{
	if (GetGameInstance() == nullptr)
	{
		return;
	...
	if ( LocalPlayer == nullptr )
	{
		UE_LOG( LogOnline, Warning, TEXT( "STrueFPSSplitScreenLobby::UnreadyPlayer: ControllerId not found: %i" ), ControllerId );
		return;
	}
...
FReply STrueFPSSplitScreenLobby::OnOkOrCancel()
{
	UTrueFPSGameViewportClient* TrueFPSViewport = Cast<UTrueFPSGameViewportClient>(GetGameInstance()->GetGameViewportClient());
...
bool STrueFPSSplitScreenLobby::ConfirmSponsorsSatisfied() const
{
	if (GetGameInstance() == nullptr)
	{
		return false;
	...
	if(OnlineSub)
	{
		...
		if(IdentityInterface.IsValid())
		{
			...
			for ( int i = 0; i < NumActiveSlots; ++i )
			{
				...
				if(Sponsor.IsValid())
				{
					return ( GetGameInstance()->FindLocalPlayerFromUniqueNetId( Sponsor ) != nullptr );
				}
			}
		}
	}
...
void STrueFPSSplitScreenLobby::OnUserCanPlay(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{
...
int32 STrueFPSSplitScreenLobby::GetNumSupportedSlots() const
{
	return FMath::Clamp( GTrueFPSSplitScreenMax, 1, MAX_POSSIBLE_SLOTS );
}

bool STrueFPSSplitScreenLobby::IsUniqueIdOnline( const FUniqueNetId& UniqueId ) const
{
	if (GetGameInstance() == nullptr)
	{
		return false;
...
FReply STrueFPSSplitScreenLobby::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	const UTrueFPSGameInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
				...
				else
				{
					...
					if ( TrueFPSViewport != nullptr )
					{
						TrueFPSViewport->ShowDialog( 
							...
							FText::GetEmpty(),
							FOnClicked::CreateRaw(this, &STrueFPSSplitScreenLobby::OnOkOrCancel),
							FOnClicked::CreateRaw(this, &STrueFPSSplitScreenLobby::OnOkOrCancel)
						);
					}
	...
	{
		if ( ExistingPlayer != nullptr && ExistingPlayer == GetGameInstance()->GetFirstGamePlayer() )
		{
...
FReply STrueFPSSplitScreenLobby::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void STrueFPSSplitScreenLobby::OnFocusLost( const FFocusEvent& InFocusEvent )
{
}

void STrueFPSSplitScreenLobby::HandleLoginUIClosedAndReady( TSharedPtr<const FUniqueNetId> UniqueId, const int UserIndex, const FOnlineError& Error )
{
	const UTrueFPSGameInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
	...
	if ( UniqueId.IsValid() )
	{
		...
		else if (!IsUniqueIdOnline(*UniqueId))
		{
			if (OnlineMode == EOnlineMode::Online)
			{
				...
				OnLoginCompleteDelegateHandle = Online::GetIdentityInterface(World)->AddOnLoginCompleteDelegate_Handle(UserIndex, FOnLoginCompleteDelegate::CreateRaw(this, &STrueFPSSplitScreenLobby::OnLoginComplete));
...
void STrueFPSSplitScreenLobby::OnLoginComplete(int32 LocalUserNum, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error)
{
...
UTrueFPSGameInstance* STrueFPSSplitScreenLobby::GetGameInstance() const
{
	...
	check( PlayerOwner->GetGameInstance() == nullptr || CastChecked< UTrueFPSGameInstance >( PlayerOwner->GetGameInstance() ) != nullptr );

	return Cast< UTrueFPSGameInstance >( PlayerOwner->GetGameInstance() );
}

FText STrueFPSSplitScreenLobby::GetPlayFindText() const
{
	return bIsJoining ? PressToFindText : PressToStartMatchText;
}

#if PLATFORM_SWITCH
FText STrueFPSSplitScreenLobby::GetPlayAsGuestText() const
{
	const UTrueFPSGameInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
