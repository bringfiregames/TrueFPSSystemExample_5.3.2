...
{
	TrueFPSHUDPCTrackerBase::Init(InContext);

	ChatStyle = &FTrueFPSStyle::Get().GetWidgetStyle<FTrueFPSChatStyle>("DefaultTrueFPSChatStyle");
	...
	ChatFont = FTrueFPSStyle::Get().GetFontStyle("TrueFPSSystem.ChatFont");
...
void SChatWidget::OnChatTextCommitted(const FText& InText, ETextCommit::Type InCommitInfo)
{
	if (InCommitInfo == ETextCommit::OnEnter)
	{
		if (GetPlayerController().IsValid() && !InText.IsEmpty())
		{
			...
			// GetPlayerController()->Say(InText.ToString()); @todo add voice speaking
