...
void STrueFPSDemoList::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			SNew(SCheckBox)
			.IsChecked(this, &STrueFPSDemoList::IsShowAllReplaysChecked)
			.OnCheckStateChanged(this, &STrueFPSDemoList::OnShowAllReplaysChecked)
			.Style(FCoreStyle::Get(), "Checkbox")
			[
				SNew(STextBlock)
				.Text(LOCTEXT("ShowAllDemos", "Show demos from all versions"))
				.TextStyle(FTrueFPSStyle::Get(), "TrueFPSSystem.DemoListCheckboxTextStyle")
			]
		]
		+SVerticalBox::Slot()
		.AutoHeight()
		[
			...
			[
				...
				.OnGenerateRow(this, &STrueFPSDemoList::MakeListViewWidget)
				.OnSelectionChanged(this, &STrueFPSDemoList::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&STrueFPSDemoList::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &STrueFPSDemoList::GetBottomText)
...
void STrueFPSDemoList::OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result )
{
...
FText STrueFPSDemoList::GetBottomText() const
{
	 return StatusText;
}
 ...
 */
void STrueFPSDemoList::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	SCompoundWidget::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
}

ECheckBoxState STrueFPSDemoList::IsShowAllReplaysChecked() const
{
...
void STrueFPSDemoList::OnShowAllReplaysChecked(ECheckBoxState NewCheckedState)
{
	EnumerateStreamsVersion = FNetworkVersion::GetReplayVersion();
...
void STrueFPSDemoList::BuildDemoList()
{
	...
	{
		ReplayStreamer->EnumerateStreams(EnumerateStreamsVersion, INDEX_NONE, FString(), TArray<FString>(), FEnumerateStreamsCallback::CreateSP(this, &STrueFPSDemoList::OnEnumerateStreamsComplete));
	}
}

/** Called when demo list building is finished */
void STrueFPSDemoList::OnBuildDemoListFinished()
{
	bUpdatingDemoList = false;
...
void STrueFPSDemoList::PlayDemo()
{
...
void STrueFPSDemoList::DeleteDemo()
{
	if (bUpdatingDemoList)
	{
		// We're still building the list
	...
	if (SelectedItem.IsValid())
	{
		...
		if ( GI != nullptr )
		{
			UTrueFPSGameViewportClient* TrueFPSGameViewportClient = Cast<UTrueFPSGameViewportClient>( GI->GetGameViewportClient() );
			...
			if ( TrueFPSGameViewportClient )
			{
				TrueFPSGameViewportClient->ShowDialog( 
					...
					FOnClicked::CreateRaw(this, &STrueFPSDemoList::OnDemoDeleteConfirm),
					FOnClicked::CreateRaw(this, &STrueFPSDemoList::OnDemoDeleteCancel)
				);
			}
		}
	}
...
FReply STrueFPSDemoList::OnDemoDeleteConfirm()
{
	if (SelectedItem.IsValid() && ReplayStreamer.IsValid())
	{
		...
		ReplayStreamer->DeleteFinishedStream(SelectedItem->StreamInfo.Name, FDeleteFinishedStreamCallback::CreateSP(this, &STrueFPSDemoList::OnDeleteFinishedStreamComplete));
	}
...
FReply STrueFPSDemoList::OnDemoDeleteCancel()
{
	UTrueFPSGameInstance* const GI = Cast<UTrueFPSGameInstance>(PlayerOwner->GetGameInstance());
...
void STrueFPSDemoList::OnDeleteFinishedStreamComplete(const FDeleteFinishedStreamResult& Result)
{
	BuildDemoList();
}

void STrueFPSDemoList::OnFocusLost(const FFocusEvent& InFocusEvent)
{
...
FReply STrueFPSDemoList::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
	return FReply::Handled().SetUserFocus(SharedThis(this), EFocusCause::SetDirectly, true);
}

void STrueFPSDemoList::EntrySelectionChanged(TSharedPtr<FDemoEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void STrueFPSDemoList::OnListItemDoubleClicked(TSharedPtr<FDemoEntry> InItem)
{
	SelectedItem = InItem;
	PlayDemo();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void STrueFPSDemoList::MoveSelection(int32 MoveBy)
{
...
FReply STrueFPSDemoList::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyboardEvent) 
{
...
TSharedRef<ITableRow> STrueFPSDemoList::MakeListViewWidget(TSharedPtr<FDemoEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
