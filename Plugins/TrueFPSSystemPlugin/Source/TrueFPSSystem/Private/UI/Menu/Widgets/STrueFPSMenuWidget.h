...
//class declare
class STrueFPSMenuWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(STrueFPSMenuWidget)
	: _PlayerOwner()
	, _IsGameMenu(false)
	{
	}
	...
	void UpdateArrows(TSharedPtr<FTrueFPSMenuItem> MenuItem);
	...
	TArray<FTrueFPSMenuInfo> MenuHistory;
...
private:
	...
	FText GetOptionText(TSharedPtr<FTrueFPSMenuItem> MenuItem) const;
	...
	const struct FTrueFPSMenuStyle *MenuStyle;
};

namespace MenuHelper
{
	FORCEINLINE void EnsureValid(TSharedPtr<FTrueFPSMenuItem>& MenuItem)
	{
		if (!MenuItem.IsValid())
		{
			MenuItem = FTrueFPSMenuItem::CreateRoot();
		}
	}

	//Helper functions for creating menu items
	FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuItem(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text));
	...
	// FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuItem(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FTrueFPSMenuItem::FOnConfirmMenuItem::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	// {
	// 	EnsureValid(MenuItem);
	// 	TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text));
	...
	template<class UserClass>
FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuItem(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, UserClass* inObj, void(UserClass::*inMethod)())
	{
		EnsureValid(MenuItem);
		TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text));
	...
	// template< class UserClass >	
	// FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuItemSP(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FTrueFPSMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	// {
	// 	EnsureValid(MenuItem);
	// 	TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text));
	...
	template<class UserClass>
FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuItemSP(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, UserClass* inObj, void(UserClass::*inMethod)())
	{
		EnsureValid(MenuItem);
		TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuOption(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text, OptionsList));
		MenuItem->SubMenu.Add(Item);
	...
	// template< class UserClass >	
	// FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuOption(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FTrueFPSMenuItem::FOnOptionChanged::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	// {
	// 	EnsureValid(MenuItem);
	// 	TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text, OptionsList));
	...
	template<class UserClass>
FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuOption(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, void (UserClass::*inMethod)(TSharedPtr<FTrueFPSMenuItem, ESPMode::ThreadSafe>, int32))
	{
		EnsureValid(MenuItem);
		TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text, OptionsList));
	...
	// template< class UserClass >	
	// FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuOptionSP(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FTrueFPSMenuItem::FOnOptionChanged::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	// {
	// 	EnsureValid(MenuItem);
	// 	TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text, OptionsList));
	...
	template<class UserClass>
FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddMenuOptionSP(TSharedPtr<FTrueFPSMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, void (UserClass::*inMethod)(TSharedPtr<FTrueFPSMenuItem, ESPMode::ThreadSafe>, int32))
	{
		EnsureValid(MenuItem);
		TSharedPtr<FTrueFPSMenuItem> Item = MakeShareable(new FTrueFPSMenuItem(Text, OptionsList));

		// Bind the method to the delegate using a shared pointer
		Item->OnOptionChanged.BindSP(inObj, inMethod);
	...
	FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddExistingMenuItem(TSharedPtr<FTrueFPSMenuItem>& MenuItem, TSharedRef<FTrueFPSMenuItem> SubMenuItem)
	{
		EnsureValid(MenuItem);
	...
	FORCEINLINE TSharedRef<FTrueFPSMenuItem> AddCustomMenuItem(TSharedPtr<FTrueFPSMenuItem>& MenuItem, TSharedPtr<SWidget> CustomWidget)
	{
		EnsureValid(MenuItem);
		MenuItem->SubMenu.Add(MakeShareable(new FTrueFPSMenuItem(CustomWidget)));
		return MenuItem->SubMenu.Last().ToSharedRef();
	}

	FORCEINLINE void ClearSubMenu(TSharedPtr<FTrueFPSMenuItem>& MenuItem)
	{
		EnsureValid(MenuItem);
	...
	/*template< class UserClass >	
	FORCEINLINE void PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, int32 UserIndex, UserClass* inObj, typename FTrueFPSMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
...
FORCEINLINE void PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, int32 UserIndex, UserClass* inObj, void (UserClass::*inMethod)())
	{
		...
		if (World)
		{
			...
			// Bind the method to the timer delegate using a shared pointer
			World->GetTimerManager().SetTimer(DummyHandle, FTimerDelegate::CreateSP(inObj, inMethod), SoundDuration, false);
		}
		else
		{
