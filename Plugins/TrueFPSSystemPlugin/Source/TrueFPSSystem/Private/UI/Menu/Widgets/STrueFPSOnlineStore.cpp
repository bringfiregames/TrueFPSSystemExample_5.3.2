...
void STrueFPSOnlineStore::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			[
				...
				.OnGenerateRow(this, &STrueFPSOnlineStore::MakeListViewWidget)
				.OnSelectionChanged(this, &STrueFPSOnlineStore::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&STrueFPSOnlineStore::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(SRichTextBlock)
				.Text(this, &STrueFPSOnlineStore::GetBottomText)
...
FText STrueFPSOnlineStore::GetBottomText() const
{
	return StatusText;
}
 ...
 */
void STrueFPSOnlineStore::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	SCompoundWidget::Tick( AllottedGeometry, InCurrentTime, InDeltaTime );
}

/** Returns logged in user */
TSharedPtr<const FUniqueNetId> STrueFPSOnlineStore::GetLoggedInUser()
{
...
void STrueFPSOnlineStore::BeginGettingOffers()
{
				...
				[this, StoreV2Int, PurchaseInt, UserId](bool bWasSuccessful, const TArray<FUniqueOfferId>& OfferIds, const FString& ErrorString)
			{
				...
				if (bWasSuccessful)
				{
					...
					for (const FOnlineStoreOfferRef& OfferRef : StoreOffers)
					{
						...
						NewOffer->Title = OfferRef->Title.IsEmptyOrWhitespace() ? NSLOCTEXT("TrueFPSOnlineStore", "DefaultOfferTitle", "EmptyTitle") : OfferRef->Title;
						NewOffer->Description = OfferRef->Description.IsEmptyOrWhitespace() ? NSLOCTEXT("TrueFPSOnlineStore", "DefaultOfferDescription", "EmptyDescription") : OfferRef->Description;
						NewOffer->Price = OfferRef->GetDisplayPrice().IsEmptyOrWhitespace() ? NSLOCTEXT("TrueFPSOnlineStore", "DefaultOfferDescription", "EmptyPrice") : OfferRef->GetDisplayPrice();
...
void STrueFPSOnlineStore::SetStoreState(EStoreState NewState)
{
	...
	switch (State)
	{
		case EStoreState::PurchasingAnOffer:
			StatusText = FText(NSLOCTEXT("TrueFPSOnlineStore", "Status", "Purchasing..."));
			break;

		case EStoreState::GettingOffers:
			StatusText = FText(NSLOCTEXT("TrueFPSOnlineStore", "Status", "Checking what's available..."));
			break;

		case EStoreState::Browsing:
			if (OfferList.Num() == 0)
			{
				StatusText = FText(NSLOCTEXT("TrueFPSOnlineStore", "Status", "No offers found - press Space to refresh"));
				break;
			}
...
void STrueFPSOnlineStore::PurchaseOffer()
{
	if (State != EStoreState::Browsing)
	{
...
void STrueFPSOnlineStore::MarkAsPurchased(const TArray<FUniqueOfferId> & Offers)
{
...
void STrueFPSOnlineStore::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply STrueFPSOnlineStore::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void STrueFPSOnlineStore::EntrySelectionChanged(TSharedPtr<FStoreEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void STrueFPSOnlineStore::OnListItemDoubleClicked(TSharedPtr<FStoreEntry> InItem)
{
	SelectedItem = InItem;
	PurchaseOffer();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void STrueFPSOnlineStore::MoveSelection(int32 MoveBy)
{
...
FReply STrueFPSOnlineStore::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent) 
{
...
TSharedRef<ITableRow> STrueFPSOnlineStore::MakeListViewWidget(TSharedPtr<FStoreEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	public:
		...
		TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName)
		{
			...
			return SNew(STextBlock)
				.Text(ItemText)
				.TextStyle(FTrueFPSStyle::Get(), "TrueFPSSystem.MenuStoreListTextStyle");
		}
