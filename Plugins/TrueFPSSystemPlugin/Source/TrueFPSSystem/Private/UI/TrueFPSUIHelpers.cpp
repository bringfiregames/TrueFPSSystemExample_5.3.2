...
#include "TrueFPSUIHelpers.h"
#include "OnlineSubsystemUtils.h"

FText TrueFPSUIHelpers::GetProfileOpenText() const
{
...
bool TrueFPSUIHelpers::ProfileOpenedUI(UWorld* World, const FUniqueNetId& Requestor, const FUniqueNetId& Requestee, const FOnProfileUIClosedDelegate* Delegate) const
{
...
FText TrueFPSUIHelpers::GetProfileSwapText() const
{
	// @todo: replace button with icon
	return NSLOCTEXT("Network", "PCSwapProfile", "Space - Switch User");
}

bool TrueFPSUIHelpers::ProfileSwapUI(UWorld* World, const int ControllerIndex, bool bShowOnlineOnly, const FOnLoginUIClosedDelegate* Delegate) const
{
	const IOnlineSubsystem* OnlineSub = Online::GetSubsystem(World);
	if (OnlineSub)
	{
